...
export TRAIN_DATASET_NAME=${TRAIN_DATASET_NAME:-DAPO-Math-17k-grpo-topk-gated-opd}
...
if [ "$ADV_ESTIMATOR" != "grpo_scaled_token_gated_opd" ]; then
    echo "grpo_topk_gated_opd.sh requires ADV_ESTIMATOR=grpo_scaled_token_gated_opd"
    exit 2
fi
if [ "$LOG_PROB_TOP_K" -le 0 ]; then
    echo "grpo_topk_gated_opd.sh requires LOG_PROB_TOP_K > 0"
    exit 2
fi
if [ "$TOP_K_STRATEGY" != "only_stu" ] && [ "$TOP_K_STRATEGY" != "union" ]; then
    echo "grpo_topk_gated_opd.sh requires TOP_K_STRATEGY=only_stu or union"
    exit 2
fi
if [ "$N_RESPONSES" -le 1 ]; then
    echo "grpo_topk_gated_opd.sh requires N_RESPONSES > 1"
    exit 2
fi
if [ "$TOPK_TOKEN_GATE_OPD_ENABLE" != "True" ]; then
    echo "grpo_topk_gated_opd.sh requires TOPK_TOKEN_GATE_OPD_ENABLE=True"
    exit 2
fi
if [ "$SAMPLED_TOKEN_GATE_OPD_ENABLE" = "True" ]; then
    echo "grpo_topk_gated_opd.sh cannot be combined with SAMPLED_TOKEN_GATE_OPD_ENABLE=True"
    exit 2
fi
