...
@register_adv_est("token_reward_direct_grpo_gated_opd")
def compute_token_reward_direct_grpo_gated_opd_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    config: Optional[AlgoConfig] = None,
    **kwargs,
    ...
    """
    Hybrid estimator for GRPO-gated OPD.

    OPD remains a top-k token reward and is gated at response level by the
    final answer reward. GRPO is computed on sampled response tokens from the
    same final answer reward and consumed by a separate actor loss branch.
    """
    if token_level_rewards.dim() != 3:
        raise ValueError(
            "token_reward_direct_grpo_gated_opd requires top-k OPD rewards with shape [B, T, K]"
        )
    if index is None:
        raise ValueError("token_reward_direct_grpo_gated_opd requires uid/index groups")

    true_reward_score = kwargs.get("true_reward_score", None)
    if true_reward_score is None:
        raise ValueError("token_reward_direct_grpo_gated_opd requires true_reward_score")

    cfg = {}
    if config is not None and hasattr(config, "get"):
        cfg = config.get("grpo_gated_opd", {}) or {}

    gate_mode = cfg.get("gate_mode", "reward_positive")
    if gate_mode != "reward_positive":
        raise ValueError(f"Unsupported grpo_gated_opd.gate_mode: {gate_mode}")

    correct_threshold = float(cfg.get("correct_threshold", 0.5))
    ...
    with torch.no_grad():
        ...
        opd_gate = (outcome_scalar > correct_threshold).to(dtype=token_level_rewards.dtype)
        opd_adv = token_level_rewards * response_mask_f.unsqueeze(-1) * opd_gate.view(-1, 1, 1) * opd_coef
        ...
        extra_tensors = {
            ...
            "grpo_gated_opd_opd_gate": opd_gate,
            "grpo_gated_opd_outcome_score": outcome_scalar.to(dtype=response_mask_f.dtype),
            "grpo_gated_opd_grpo_scalar_advantage": grpo_scalar_adv,
            "grpo_gated_opd_opd_adv_abs_mean_token": opd_adv.abs().mean(dim=-1),
            "grpo_gated_opd_grpo_adv_abs_token": grpo_adv.abs(),
        }
